...
import { chatWithAI } from './secondme';
...
export async function executeNextRound(conversationId: string): Promise<Conversation> {
  ...
  const speakerData = await kvGet<StoredUser>(KEYS.user(currentSpeaker.userId));
  if (!speakerData) {
    conv.status = 'error';
    await kvSet(KEYS.conversation(conversationId), conv);
    throw new Error(`Player ${currentSpeaker.name} data not found`);
  }
  ...
  try {
    ...
    const result = await chatWithAI(
      speakerData.tokens.accessToken,
      messageToSend,
      conv.currentRound === 0 || conv.currentRound === 1 ? systemPrompt : undefined,
      undefined
    );
